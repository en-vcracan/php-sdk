...
class Gp3DSProvider extends RestGateway implements ISecure3dProvider
{
    ...
    public function processSecure3d(Secure3dBuilder $builder)
    {
        ...
        if ($transType === TransactionType::VERIFY_ENROLLED) {
            ...
            $request['payer'] = $this->maybeSetKey(
                $request['payer'],
                'email',
                $builder->getCustomerEmail() ? $builder->getCustomerEmail() : null
            );
    ...
    private function mapResponse($rawResponse)
    {
        ...
        // check enrolled
        $secureEcom->serverTransactionId = $doc['server_trans_id'] ? $doc['server_trans_id'] : null;
        if (array_key_exists('enrolled', $doc)) {
            $secureEcom->enrolled = (bool)$doc['enrolled'];
        }
        $secureEcom->issuerAcsUrl =
            ($doc['method_url'] ? $doc['method_url'] : null) .
            ($doc['challenge_request_url'] ? $doc['challenge_request_url'] : null);

        // get authentication data
        $secureEcom->acsTransactionId = $doc['acs_trans_id'] ? $doc['acs_trans_id'] : null;
        $secureEcom->directoryServerTransactionId = $doc['ds_trans_id'] ? $doc['ds_trans_id'] : null;
        $secureEcom->authenticationType = $doc['authentication_type'] ? $doc['authentication_type'] : null;
        $secureEcom->authenticationValue = $doc['authentication_value'] ? $doc['authentication_value'] : null;
        $secureEcom->eci = $doc['eci'] ? $doc['eci'] : null;
        $secureEcom->status = $doc['status'] ? $doc['status'] : null;
        $secureEcom->statusReason = $doc['status_reason'] ? $doc['status_reason'] : null;
        $secureEcom->authenticationSource = $doc['authentication_source'] ? $doc['authentication_source'] : null;
        $secureEcom->messageCategory = $doc['message_category'] ? $doc['message_category'] : null;
        $secureEcom->messageVersion = $doc['message_version'] ? $doc['message_version'] : null;
        ...
        $secureEcom->cardHolderResponseInfo =
            $doc['cardHolder_response_info'] ? $doc['cardHolder_response_info'] : null;

        // device_render_options
        if (array_key_exists('device_render_options', $doc)) {
            $renderOptions = $doc['device_render_options'];
            $secureEcom->sdkInterface = $renderOptions['sdk_interface'] ? $renderOptions['sdk_interface'] : null;
            $secureEcom->sdkUiType = $renderOptions['sdk_ui_type'] ? $renderOptions['sdk_ui_type'] : null;
        }
        ...
        if (array_key_exists('message_extension', $doc)) {
            ...
            $secureEcom->messageExtensionId = $doc['message_extension']['id'] ? $doc['message_extension']['id'] : null;
            $secureEcom->messageExtensionName =
                $doc['message_extension']['name'] ? $doc['message_extension']['name'] : null;
        }

        // versions
        $secureEcom->directoryServerEndVersion =
            $doc['ds_protocol_version_end'] ? $doc['ds_protocol_version_end'] : null;
        $secureEcom->directoryServerStartVersion =
            $doc['ds_protocol_version_start'] ? $doc['ds_protocol_version_start'] : null;
        $secureEcom->acsEndVersion = $doc['acs_protocol_version_end'] ? $doc['acs_protocol_version_end'] : null;
        $secureEcom->acsStartVersion = $doc['acs_protocol_version_start'] ? $doc['acs_protocol_version_start'] : null;
        ...
        if (array_key_exists('method_data', $doc)) {
            ...
            $secureEcom->payerAuthenticationRequest =
                $methodData['encoded_method_data'] ? $methodData['encoded_method_data'] : null;
        } elseif (array_key_exists('encoded_creq', $doc)) {
            $secureEcom->payerAuthenticationRequest = $doc['encoded_creq'] ? $doc['encoded_creq'] : null;
        }
